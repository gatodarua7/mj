...
class TestTipoVinculoEndpoint(SiapenTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.data = {"nome": "TIPO VÍNCULO", "ativo": True}
        self.url = f"/api/vinculos/tipo-vinculo/"
        super(TestTipoVinculoEndpoint, self).setUp()
    ...
    def test_e_create(self):
        ...
        """
        data = self.data
        data["nome"] = "TIPO VINCULO TESTE INATIVO"
        data["ativo"] = False
        resp = self.client.post(self.url, data=data)
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.format_print(metodo='create')
