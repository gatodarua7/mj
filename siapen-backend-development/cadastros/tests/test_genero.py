...
class TestGeneroEndpoint(SiapenTestCase):
    fixtures = ["fixtures/usuarios/usuario.json", "fixtures/cadastro/genero.json"]

    def setUp(self) -> None:
        self.entidade = "Gênero"
        self.data = {"descricao": "GÊNERO", "ativo": True}
        self.url = f"/api/cadastros/generos/"
    ...
    def test_e_create(self):
        ...
        """
        data = self.data
        data["descricao"] = "GENERO TESTE INATIVO"
        data["ativo"] = False
        resp = self.client.post(self.url, data=data)
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.format_print(metodo="create")
    ...
    def test_c_update(self):
        ...
        if status.is_success(resp.status_code):
            resp_json = resp.json()
            resp_json["descricao"] = "TESTE INATIVO"
