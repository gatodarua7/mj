...
class TestServidorEndpoint(SiapenTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.data = {"nome": "JOSE MARIA", "nome_social": "", "cpf": "79968464058", "rg": "",
                    "orgao_expedidor": "", "genero": "96637461-43cb-4d55-893b-c09fe514ecf7",
                    "raca": "", "estado_civil": "3c2c263e-0713-4bf1-a707-903fe601e6ae",
                    "nacionalidade": [1], "estado": "", "naturalidade": "",
                    "nome_mae": "MARIA", "nome_pai": "JOSE",
                    ...
                    "documentos": [],
                    "telefones_funcionais": [{"id":"eb43b191-cfad-4011-879d-b2da2466e8ea",
                                                "numero": 9869,
                                                "tipo": "RESIDENCIAL",
    ...
    def test_d_update(self):
        ...
        resp = self.client.post(self.base_url+self.url, data=json.dumps(data), content_type='application/json', proxies=self.proxies,headers=self.headers)
        if status.is_success(resp.status_code):
            resp_json = resp.json()
            url = f'{self.base_url+self.url}{resp_json["id"]}/'
            resp = requests.put(url, data=json.dumps(data), proxies=self.proxies, headers=self.headers)
