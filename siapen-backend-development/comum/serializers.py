...
class EnderecoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Endereco
        ordering = ['id']
        fields = ['id', 'logradouro', 'bairro', 'numero', 'complemento', 'cep',
                  'estado', 'cidade', 'andar', 'sala', 'observacao', 'ativo',
                  'estado_nome', 'cidade_nome']
...
class TelefoneSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Telefone
        ordering = ['id']
        fields = ['id', 'numero', 'tipo', 'observacao',
                  'andar', 'sala', 'privado']

    def __init__(self, *args, **kwargs):
        super(TelefoneSerializer, self).__init__(*args, **kwargs)

        self.fields['numero'].error_messages['blank'] = mensagens.MSG2.format(
            u'numero')
        self.fields['tipo'].error_messages['blank'] = mensagens.MSG2.format(
            u'tipo')
    ...
    def validate(self, data):
        ...
        if (data.get('tipo') and data.get('tipo') != 'RAMAL') and (data.get('andar') or data.get('sala')):
