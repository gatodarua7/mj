...
class EscoltasViewSet(LoggingMixin, viewsets.ModelViewSet, Base):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not requisicao.get("pedidos_inclusao"):
            return Response(
                {"non_field_errors": mensagens.MSG36},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if requisicao.get("tipo_aeronave") == "COMERCIAL" and not requisicao.get(
            "voos"
        ):
            return Response(
                {"tipo_aeronave": mensagens.MSG_VOOS},
                status=status.HTTP_400_BAD_REQUEST,
            )
        with transaction.atomic():
            for p_inclusao in requisicao.get("pedidos_inclusao"):
                ...
                if not self.check_pedido_aguardando_escolta(pedido):
                    return Response(
                        {
                            "non_field_errors": "Há pedidos de inclusão nesta escolta já associados a outro agendamento."
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                erro_data = dict()
                ...
                pedido.save()
            self.request.data["fase_escolta_aerea"] = (
                "AGENDADA" if requisicao.get("data_inicio_aerea") else ""
            )
            self.request.data["fase_escolta_terrestre"] = (
                "AGENDADA" if requisicao.get("data_inicio_terrestre") else ""
            )
            self.request.data["numero_escolta"] = (
                self.get_numero_escolta()
                if requisicao.get("responsavel") == "DEPEN"
                else "-"
            )
    ...
    def update(self, request, pk, *args, **kwargs):
        ...
        pedidos_inclusao_list = [
            UUID(p_inclusao["id"]) for p_inclusao in requisicao.get("pedidos_inclusao")
        ]
        servidor_aerea_list = list()
        if requisicao.get("servidores_escolta_aerea"):
            servidor_aerea_list = [
                s.get("id") for s in requisicao.get("servidores_escolta_aerea")
            ]
        servidor_terrestre_list = list()
        if requisicao.get("servidores_escolta_terrestre"):
            servidor_terrestre_list = [
                s.get("id") for s in requisicao.get("servidores_escolta_terrestre")
            ]
        ...
        with transaction.atomic():
            try:
                for p in pedidos_inclusao_list:
                    ...
                    erro_data = dict()
                    erro_data.update(
                        self.check_data_inicio_escolta(requisicao, p_inclusao)
                    )
                    if erro_data:
                        return Response(erro_data, status=status.HTTP_400_BAD_REQUEST)
                ...
                for escolta in Escoltas.objects.filter(Q(id=UUID(pk))):
                    for pedido in escolta.pedidos_inclusao.all():
                        if pedido.pk not in pedidos_inclusao_list:
                            pedido_inclusao = PedidoInclusao.objects.select_for_update().get(
                                pk=pedido.pk
                            )
                            pedido_inclusao.aguardando_escolta = True
                            pedido_inclusao.save()
                    escolta.pedidos_inclusao.clear()
                    for pedidos_add in PedidoInclusao.objects.filter(
                        id__in=pedidos_inclusao_list
                    ):
                    ...
                    for servidor in Servidor.objects.filter(
                        id__in=servidor_terrestre_list
                    ):
            ...
            self.request.data["fase_escolta_aerea"] = (
                requisicao.get("fase_escolta_aerea")
                if requisicao.get("data_inicio_aerea")
                else ""
            )
            self.request.data["fase_escolta_terrestre"] = (
                requisicao.get("fase_escolta_terrestre")
    ...
    def destroy(self, request, pk, *args, **kwargs):
        if not Base().check_registro_exists(Escoltas, pk):
            return Response(
                {"detail": mensagens.NAO_ENCONTRADO}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def filter_queryset(self, queryset):
        ...
        for query in Escoltas.objects.filter(Q(excluido=False)):
            ...
            escolta_list.append(
                formata_data(trata_campo(query.data_inicio_aerea))
                if query.data_inicio_aerea
                else ""
            )
            escolta_list.append(
                formata_data(trata_campo(query.data_fim_aerea))
                if query.data_fim_aerea
                else ""
            )
            escolta_list.append(
                formata_data(trata_campo(query.data_inicio_terrestre))
                if query.data_inicio_terrestre
                else ""
            )
            escolta_list.append(
                formata_data(trata_campo(query.data_fim_terrestre))
                if query.data_fim_terrestre
                else ""
            )
            escolta_list.append(
                formata_hora(query.hora_inicio_aerea) if query.hora_inicio_aerea else ""
            )
            escolta_list.append(
                formata_hora(query.hora_fim_aerea) if query.hora_fim_aerea else ""
            )
            escolta_list.append(
                formata_hora(query.hora_inicio_terrestre)
                if query.hora_inicio_terrestre
                else ""
            )
            escolta_list.append(
                formata_hora(query.hora_fim_terrestre)
                if query.hora_fim_terrestre
                else ""
            )
            ...
            for servidor in query.servidores_escolta_terrestre.all():
                ...
                escolta_list.append(
                    trata_campo(servidor.cargos.cargo if servidor.cargos else "")
                )
            ...
            for pedido in query.pedidos_inclusao.all():
                ...
                escolta_list.append(
                    trata_campo(pedido.unidade.nome if pedido.unidade else "")
                )
                escolta_list.append(
                    trata_campo(
                        pedido.estado_solicitante.nome
                        if pedido.estado_solicitante
                        else ""
                    )
                )
    ...
    def check_pedido_aguardando_escolta(self, pedido):
        return PedidoInclusao.objects.filter(
            id=pedido.pk, aguardando_escolta=True
        ).exists()
    ...
    def get_numero_escolta(self):
        ...
        ano = rf"/{now.year}"
        try:
            escolta = Escoltas.objects.filter(
                responsavel="DEPEN", numero_escolta__contains=ano
            ).latest("created_at")
        except Exception:
    ...
    def check_data_inicio_escolta(self, requisicao, pedido):
        ...
        if requisicao.get("data_inicio_aerea"):
            data_inicio_aereo = converte_string_data_hora(
                requisicao.get("data_inicio_aerea"), requisicao.get("hora_inicio_aerea")
            )
        ...
        if requisicao.get("data_inicio_terrestre"):
            data_inicio_terrestre = converte_string_data_hora(
                requisicao.get("data_inicio_terrestre"),
    ...
    def get_pedidos_inclusao(self, request):
        lista_pedidos = list()
        pedido_list = (
            self.request.query_params.get("pedido").split(",")
            if self.request.query_params.get("pedido")
            else []
        )
    ...
    @action(detail=False, methods=["get"], url_path="calendario", url_name="calendario")
    def get_calendario(self, request):
        lista_escolta = list()
        data_inicio = datetime.strptime(
            self.request.query_params.get("data_inicio"), "%Y-%m-%d"
        ).date()
        data_fim = datetime.strptime(
            self.request.query_params.get("data_fim"), "%Y-%m-%d"
        ).date()

        for escolta in Escoltas.objects.filter(
            (
                Q(data_inicio_aerea__range=[data_inicio, data_fim])
                ...
                | Q(
                    data_inicio_terrestre__lte=data_inicio,
                    data_fim_terrestre__gte=data_inicio,
                )
            )
            & Q(excluido=False)
        ):
            ...
            data_inicio_aereo = (
                concatena_data_hora(
                    escolta.data_inicio_aerea, escolta.hora_inicio_aerea
                )
                if escolta.data_inicio_aerea
                else None
            )
            data_fim_aereo = (
                concatena_data_hora(escolta.data_fim_aerea, escolta.hora_fim_aerea)
                if escolta.data_fim_aerea
                else None
            )
            data_inicio_terrestre = (
                concatena_data_hora(
                    escolta.data_inicio_terrestre, escolta.hora_inicio_terrestre
                )
                if escolta.data_inicio_terrestre
                else None
            )
            data_fim_terrestre = (
                concatena_data_hora(
                    escolta.data_fim_terrestre, escolta.hora_fim_terrestre
                )
                if escolta.data_fim_terrestre
                else None
            )
            ...
            escolta_dict = {
                ...
                "start": dia_inicio[:10]
                if dia_fim[:10] != dia_inicio[:10]
                else dia_inicio,
                "hora_inicio": dia_inicio[11:],
                "end": dia_fim[:10] if dia_fim[:10] != dia_inicio[:10] else dia_fim,
                "fase": fase,
                "unidade_nome": sorted(set(unidade_nome))
                if unidade_nome
                else unidade_nome,
                "tipo_escolta_nome": escolta.get_tipo_escolta_display()
                if escolta.tipo_escolta
                else None,
            }
    ...
    def get_pedidos(self, queryset):
        ...
        for query in queryset:
            dict_pedido = {
                ...
                "estado_solicitante_nome": query.estado_solicitante.nome
                if query.estado_solicitante
                else "",
    ...
    def perform_create(self, serializer, **kwargs):
        ...
        kwargs["pedidos_inclusao"] = [
            UUID(p_inclusao["id"]) for p_inclusao in requisicao.get("pedidos_inclusao")
        ]
        if requisicao.get("servidores_escolta_aerea"):
            kwargs["servidores_escolta_aerea"] = [
                s["id"] for s in requisicao.get("servidores_escolta_aerea")
            ]
        if requisicao.get("servidores_escolta_terrestre"):
            kwargs["servidores_escolta_terrestre"] = [
                s["id"] for s in requisicao.get("servidores_escolta_terrestre")
            ]
        ...
        for voo in voos:
            if len(voo) > 20:
                raise ValidationError(
                    {"non_field_errors": mensagens.MSG_TAM_VOOS},
    ...
    def update_many_fields(self, pk):
        ...
        if requisicao.get("voos"):
            voos = [voo.get("numero_voo") for voo in requisicao.get("voos")]
        voos_base = VoosEscolta.objects.filter(Q(escolta_id=pk)).values_list(
            "voo", flat=True
        )
        if voos:
            for voos_escolta in VoosEscolta.objects.filter(
                ~Q(voo__in=voos) & Q(escolta_id=pk)
            ):
            ...
            if not voos_base:
                for voo in voos:
                    ...
                    VoosEscolta.objects.create(
                        voo=voo, escolta_id=pk, usuario_cadastro=user.get_user(self)
                    )
            else:
                for voo in voos:
                    if len(voo) > 20:
