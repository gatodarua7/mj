...
from util.busca import (
    formata_data_escolta_aerea,
    formata_data_escolta_terrestre,
    get_nome_semana,
    concatena_data_hora,
...
class EscoltasSerializer(FlexFieldsModelSerializer):
    ...
    def validate(self, data):
        erro = dict()
        if not self.check_periodo_escolta(
            data, "aerea"
        ) and not self.check_periodo_escolta(data):
            raise serializers.ValidationError(
                {"non_field_errors": mensagens.MSG_ESCOLTA}
            )
        ...
        if self.check_periodo_escolta(data):
            ...
            if not erro.get("data_fim_terrestre") and not erro.get(
                "hora_fim_terrestre"
            ):
                erro.update(self.check_data_fim_escolta(data))
        if (
            self.check_periodo_escolta(data, "aerea")
            and self.check_periodo_escolta(data)
        ) and (
            not self.check_periodo_escolta(erro, "aerea")
            and not self.check_periodo_escolta(erro)
        ):
            erro.update(self.check_data_fim_escolta_terrestre(data))
        if data.get("responsavel") == "DEPEN" and self.check_periodo_escolta(
            data, "aerea"
        ):
    ...
    def check_periodo_escolta(self, data, tipo_escolta="terrestre"):
        ...
        return (
            data.get(f"data_inicio_{tipo_escolta}")
            or data.get(f"data_fim_{tipo_escolta}")
            or data.get(f"hora_inicio_{tipo_escolta}")
            or data.get(f"hora_fim_{tipo_escolta}")
        )

    def check_fields_periodo_escolta(self, data, tipo_escolta="terrestre"):
        erro = dict()
        if not data.get(f"data_inicio_{tipo_escolta}"):
            erro[f"data_inicio_{tipo_escolta}"] = mensagens.MSG2.format(
                "data de início"
            )
        ...
        if not data.get(f"hora_inicio_{tipo_escolta}"):
            erro[f"hora_inicio_{tipo_escolta}"] = mensagens.MSG2.format(
                "hora de início"
            )
    ...
    def check_data_fim_escolta(self, data, tipo_escolta="terrestre"):
        erro = dict()
        if data.get(f"data_inicio_{tipo_escolta}") and data.get(
            f"data_fim_{tipo_escolta}"
        ):
            if data.get(f"data_inicio_{tipo_escolta}") > data.get(
                f"data_fim_{tipo_escolta}"
            ):
                erro[
                    f"data_fim_{tipo_escolta}"
                ...
                data.get(f"data_inicio_{tipo_escolta}")
                == data.get(f"data_fim_{tipo_escolta}")
            ) and data.get(f"hora_inicio_{tipo_escolta}") >= data.get(
                f"hora_fim_{tipo_escolta}"
            ):
                erro[
                    f"hora_fim_{tipo_escolta}"
                ] = "Hora fim deve ser maior que a hora de início."
        return erro
    ...
    def check_data_fim_escolta_terrestre(self, data):
        ...
        if data.get(f"data_fim_aerea") > data.get(f"data_fim_terrestre"):
            erro[
                f"data_fim_terrestre"
            ] = "Data fim terrestre não pode ser menor que a data fim aérea."
        elif (
            data.get(f"data_fim_aerea") == data.get(f"data_fim_terrestre")
        ) and data.get(f"hora_fim_aerea") >= data.get(f"hora_fim_terrestre"):
            erro[
                f"hora_fim_terrestre"
            ] = "Hora fim terrestre deve ser maior que a hora fim aérea."
        return erro
    ...
    def get_inicio_aereo(self, obj):
        return (
            concatena_data_hora(obj.data_inicio_aerea, obj.hora_inicio_aerea)
            if obj.data_inicio_aerea
            else None
        )
    ...
    def get_fim_aereo(self, obj):
        return (
            concatena_data_hora(obj.data_fim_aerea, obj.hora_fim_aerea)
            if obj.data_fim_aerea
            else None
        )

    def get_inicio_terrestre(self, obj):
        return (
            concatena_data_hora(obj.data_inicio_terrestre, obj.hora_inicio_terrestre)
            if obj.data_inicio_terrestre
            else None
        )
    ...
    def get_fim_terrestre(self, obj):
        return (
            concatena_data_hora(obj.data_fim_terrestre, obj.hora_fim_terrestre)
            if obj.data_fim_terrestre
            else None
        )
    ...
    def get_dia_semana_inicio_terrestre(self, obj):
        return (
            get_nome_semana(obj.data_inicio_terrestre)
            if obj.data_inicio_terrestre
            else None
        )
    ...
    def get_dia_semana_fim_terrestre(self, obj):
        return (
            get_nome_semana(obj.data_fim_terrestre) if obj.data_fim_terrestre else None
        )
    ...
    def check_status_escolta_aerea(self, obj):
        now = datetime.now()
        status = "AGENDADA"
        data_hora_inicio = concatena_data_hora(
            obj.data_inicio_aerea, obj.hora_inicio_aerea
        )
        data_hora_fim = concatena_data_hora(obj.data_fim_aerea, obj.hora_fim_aerea)
        if now > cast_string_in_datetime(data_hora_fim):
            status = "FINALIZADA"
        elif now > cast_string_in_datetime(
            data_hora_inicio
        ) and now < cast_string_in_datetime(data_hora_fim):
    ...
    def check_status_escolta_terrestre(self, obj):
        ...
        status = "AGENDADA"
        data_hora_inicio = concatena_data_hora(
            obj.data_inicio_terrestre, obj.hora_inicio_terrestre
        )
        data_hora_fim = concatena_data_hora(
            obj.data_fim_terrestre, obj.hora_fim_terrestre
        )
        if now > cast_string_in_datetime(data_hora_fim):
            status = "FINALIZADA"
        elif now > cast_string_in_datetime(
    ...
    def get_unidade_nome(self, obj):
        unidade_nome = [
            pedido.unidade.nome
            for pedido in obj.pedidos_inclusao.all()
            if pedido.unidade
        ]
