...
class TestReligiaoEndpoint(SiapenTestCase):
    fixtures = ["fixtures/usuarios/usuario.json", "fixtures/social/religiao.json"]

    def setUp(self) -> None:
        self.entidade = "RELIGIÂO"
        self.data = {"nome": "RELIGIÂO TESTE", "ativo": True}
        self.url = f"/api/social/religiao/"
        super(TestReligiaoEndpoint, self).setUp()
    ...
    def test_e_create(self):
        ...
        """
        data = self.data
        data["nome"] = "RELIGIÂO TESTE INATIVO"
        data["ativo"] = False
        resp = self.client.post(self.url, data=data)
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.format_print(metodo="create")
    ...
    def test_b_list(self):
        ...
        url = f"{self.url}?search=RELIGIÂO"
    ...
    def test_c_list(self):
        ...
        """
        url = f"{self.url}?search=RELIGIAO"
        resp = self.client.post(self.url, data=self.data)
        if status.is_success(resp.status_code):
            resp_json = resp.json()
            resp = self.client.get(url)
    ...
    def test_c_delete(self):
        ...
        """
        url = f"{self.base_url+self.url}1c063b46-2b96-41a9-831d-4fed804ae05e/"
