...
class TestEstadoCivilEndpoint(SiapenTestCase):
    fixtures = ["fixtures/usuarios/usuario.json", "fixtures/social/estado_civil.json"]

    def setUp(self) -> None:
        self.entidade = "Estado_Civil"
        self.data = {"nome": "ESTADO CIVIL TESTE", "ativo": True}
        self.url = f"/api/social/estado-civil/"
        super(TestEstadoCivilEndpoint, self).setUp()
    ...
    def test_e_create(self):
        ...
        """
        data = self.data
        data["nome"] = "ESTADO CIVIL TESTE INATIVO"
        data["ativo"] = False
        resp = self.client.post(self.url, data=data)
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.format_print(metodo="create")
