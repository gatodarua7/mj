...
class TestProfissaoEndpoint(SiapenTestCase):
    fixtures = ["fixtures/usuarios/usuario.json", "fixtures/social/profissao.json"]

    def setUp(self) -> None:
        self.entidade = "PROFISSÂO"
        self.data = {"nome": "PROFISSÂO TESTE", "ativo": True}
        self.url = f"/api/social/profissao/"
        super(TestProfissaoEndpoint, self).setUp()
    ...
    def test_e_create(self):
        ...
        """
        data = self.data
        data["nome"] = "PROFISSÂO TESTE INATIVO"
        data["ativo"] = False
        resp = self.client.post(self.url, data=data)
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.format_print(metodo="create")
    ...
    def test_b_list(self):
        ...
        url = f"{self.url}?search=PROFISSÂO"
    ...
    def test_c_list(self):
        ...
        """
        url = f"{self.url}?search=PROFISSAO"
        resp = self.client.post(self.url, data=self.data)
        if status.is_success(resp.status_code):
            resp_json = resp.json()
            resp = self.client.get(url)
    ...
    def test_c_delete(self):
        ...
        """
        url = f"{self.base_url+self.url}b790d23e-9e25-4054-915e-5ff90e628bf1/"
