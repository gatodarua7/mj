...
class TestOrientacaoSexualEndpoint(SiapenTestCase):
    fixtures = [
    ...
    def setUp(self) -> None:
        self.entidade = "Orientação_Sexual"
        self.data = {"nome": "ORIENTÃO TESTE", "ativo": True}
        self.url = f"/api/social/orientacao-sexual/"
        super(TestOrientacaoSexualEndpoint, self).setUp()
    ...
    def test_e_create(self):
        ...
        """
        data = self.data
        data["nome"] = "ORIENTAÇÃO TESTE INATIVO"
        data["ativo"] = False
        resp = self.client.post(self.url, data=data)
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.format_print(metodo="create")
    ...
    def test_b_list(self):
        ...
        url = f"{self.url}?search=ORIENTAÇÂO"
    ...
    def test_c_list(self):
        ...
        """
        url = f"{self.url}?search=ORIENTACAO"
        resp = self.client.post(self.url, data=self.data)
        if status.is_success(resp.status_code):
            resp_json = resp.json()
            resp = self.client.get(url)
    ...
    def test_c_delete(self):
        ...
        """
        url = f"{self.base_url+self.url}fd9341f3-4c70-43a2-9e12-1e2da2d0ae4e/"
