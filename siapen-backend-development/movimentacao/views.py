...
from movimentacao.serializers import (FasesPedidoSerializer,
...
class FasesPedidoViewSet(LoggingMixin, viewsets.ModelViewSet, Base):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if self.check_fase_cgin(request.data):
            return Response({"non_field_errors": "A fase marcada como CGIN deve ser única e não está marcada como ultima fase."}, 
                            status=status.HTTP_400_BAD_REQUEST)
        
        if not self.check_ultima_fase_is_valid(request.data):
            return Response({"non_field_errors": "A fase marcada como ULTIMA FASE deve possuir a opção de FAVORÁVEL ou DESFAVORÁVEL."}, 
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_grupos(self, request):
        grupo_fase_list = [
            {
                "grupo": "EMERGENCIAL", "total_fases": FasesPedido.objects.filter(
                    grupo='EMERGENCIAL', excluido=False).count()}, {
                "grupo": "DEFINITIVO", "total_fases": FasesPedido.objects.filter(
                    grupo='DEFINITIVO', excluido=False).count()}]
    ...
    def qtd_fase_inicial_incorreta(self, fases):
        try:
            fase_inicial = [i for i,_ in enumerate(fases) if _.get('fase_inicial') is True]
    ...
    def check_fase_inicial(self, fases):
        try:
            fase_inicial = [1 for i,_ in enumerate(fases) if _.get('fase_inicial') is True]
    ...
    def check_ordem_ultimas_fase(self, fases):
        try:
            ultima_fase = [i for i,_ in enumerate(fases) if _.get('fase') == 'ULTIMA_FASE']
            fase_intermediaria = [i for i,_ in enumerate(fases) if _.get('fase') != 'ULTIMA_FASE']
            return (ultima_fase and fase_intermediaria) and (fase_intermediaria[-1] > ultima_fase[0])
        except Exception:
            return Response(
                {"non_field_errors": mensagens.MSG_ERRO},
                status=status.HTTP_400_BAD_REQUEST,
    ...
    def check_fase_cgin(self, fases):
        try:
            ultima_fase = [i for i,_ in enumerate(fases) if _.get('fase') == 'ULTIMA_FASE']
            cgin_ultima_fase = [i for i,_ in enumerate(fases) if _.get('ultima_fase') and _.get('fase') == 'CGIN']
            cgin_fase = [i for i,_ in enumerate(fases) if _.get('fase') == 'CGIN']
            return cgin_fase and (len(cgin_fase) > 1 or cgin_ultima_fase or (ultima_fase and cgin_fase[-1] > ultima_fase[0]))
        except Exception:
            return Response(
                {"non_field_errors": mensagens.MSG_ERRO},
                status=status.HTTP_400_BAD_REQUEST,
    ...
    def check_item(self, fases):
        try:
            queryset = FasesPedido.objects.filter(grupo=fases['grupo'], 
                                                    excluido=False,
                                                    ativo=True).order_by('ordem').last()
            if queryset:
    ...
    def check_item_primeira_fase(self, fases):
        ...
        try:
            queryset = FasesPedido.objects.filter(grupo=fases['grupo'], 
                                                    excluido=False,
                                                    ativo=True).count()
            return queryset == 0 and fases['ordem'] == 1 and (fases.get('primeira_fase') 
    ...
    def check_vinculos(self, pk):
        return PedidoInclusao.objects.filter(Q(fase_pedido_id=pk, excluido=False)).exists()
...
class PedidoInclusaoViewSet(LoggingMixin, viewsets.ModelViewSet, Base):
    ...
    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    ordering_fields = ("nome", "created_at", "estado_solicitante__nome",
                         "tipo_inclusao", "fase_pedido__nome", "unidade__nome",
                          "data_movimentacao", "tipo_escolta")
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if data_nascimento and not self.check_maioridade_penal(
                    data_nascimento):
                return Response(
            ...
            if tipo_inclusao:
                try:
                    FasesPedido.objects.get(
                        grupo=tipo_inclusao,  fase_inicial=True)
                except Exception:
                    return Response(
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            if requisicao.get("motivo_exclusao") and not self.check_fase_iniciado_exists(requisicao.get('id')):
    ...
    def filter_queryset(self, queryset):
        ...
        for query in PedidoInclusao.objects.filter(Q(excluido=False)):
            ...
            pedido_list.append(formata_data_hora(query.data_movimentacao) if query.data_movimentacao else "")
            pedido_list.append(trata_campo(query.fase_pedido.nome) if query.fase_pedido else "")
            pedido_list.append(trata_campo(query.unidade.nome) if query.unidade else "")
            pedido_list.append(trata_campo(query.genero.descricao) if query.genero else "")
            ...
            pedido_list.append(trata_campo(query.regime_prisional.nome) if query.regime_prisional else "")
            pedido_list.append(trata_campo(query.naturalidade.nome) if query.naturalidade else "")
            ...
            pedido_list.extend([trata_campo(necessidade.nome) for necessidade in query.necessidade_especial.all()])

            pedido_list.extend([trata_campo(nacionalidade.nome)
                for nacionalidade in query.nacionalidade.all()
            ])

            pedido_list.extend([trata_campo(vulgo)
                for vulgo in VulgosThroughModel.objects.filter(pedido_inclusao_id=query.id)
            ])

            pedido_list.extend([
                trata_campo(outros_nomes.nome)
                for outros_nomes in PedidoInclusaoOutroNome.objects.filter(pedido_inclusao_id=query.id, excluido=False)
            ])
            ...
            for motivo in PedidoInclusaoMotivos.objects.filter(pedido_inclusao_id=query.id):
                pedido_list.append(trata_campo(motivo.get_norma_juridica_display()))
                pedido_list.append(trata_campo(motivo.titulo.nome))
                
                pedido_list.extend([(trata_campo(descricao_motivo.descricao)) for descricao_motivo in motivo.descricao.all()])
            ...
            if not qs and (busca_sem_especial in numero_sei or busca_sem_especial in cpf):
                qs = PedidoInclusao.objects.filter(pk=query.pk)
        ...
        if parametros_busca.get("fase") and parametros_busca["fase"] == 'cgin':
            fases_list = FasesPedido.objects.filter(fase='CGIN', excluido=False).values_list('id', flat=True)
            ...
            fases_list = FasesPedido.objects.filter(fase='ULTIMA_FASE', ultima_fase='DEFERIDO', excluido=False).values_list('id', flat=True)
            queryset = queryset.filter(fase_pedido__in=fases_list, aguardando_escolta=True, excluido=False)
    ...
    def check_interno_exists(self, requisicao):
        ...
        return Interno.objects.filter(Q(cpf__iexact=cpf, excluido=False, ativo=True)).exists()
    ...
    def check_fase_exists(self, requisicao):
        ...
        return FasesPedido.objects.filter(
            Q(grupo__exact=grupo, fase_inicial=True, excluido=False)).exists()

    def check_fase_iniciado_exists(self, id):
        return PedidoInclusao.objects.filter(Q(pk=id, fase_pedido__fase_inicial=True)).exists()

    def perform_create(self, serializer, **kwargs):
        kwargs["usuario_cadastro"] = user.get_user(self)
        kwargs["created_at"] = datetime.now()
        kwargs["fase_pedido_id"] = FasesPedido.objects.get(grupo=self.request.data['tipo_inclusao'],  fase_inicial=True).pk
        serializer.save(**kwargs)
    ...
    def create_many_fields(self, serializer):
        ...
        try:
            ...
            motivos_inclusao = ast.literal_eval(
                requisicao.get("motivos_inclusao"))
        except Exception:
            motivos_inclusao = (
        ...
        if vulgo_nome:
            for nome_vulgo in vulgo_nome:
                if nome_vulgo.strip():
                    ...
                    id_vulgo, created = Vulgo.objects.get_or_create(
                        nome=nome_vulgo)
        ...
        if motivos_inclusao:
            ...
            [pedido_motivo.descricao.add(norma) for norma in NormasJuridicas.objects.filter(id__in=motivo['descricao'])]
        fase = FasesPedido.objects.get(grupo=serializer.data["tipo_inclusao"],  fase_inicial=True)
        PedidoInclusaoMovimentacao.objects.create(pedido_inclusao_id=serializer.data["id"],
                                                                    usuario_cadastro=user.get_user(self),
                                                                    fase_pedido=fase,
    ...
    def update_many_fields(self, pk):
        ...
        if vulgo_nome:
            for nome_vulgo in vulgo_nome:
                if nome_vulgo.strip():
                    ...
                    VulgosThroughModel.objects.get_or_create(pedido_inclusao_id=pk, vulgo_id=id_vulgo.id)
            novos_vulgos = Vulgo.objects.filter(nome__in=vulgo_nome).values_list('id', flat=True)
            for vulgo in VulgosThroughModel.objects.filter(~Q(vulgo_id__in=novos_vulgos) & Q(pedido_inclusao_id=pk)):
                   VulgosThroughModel.objects.filter(Q(pk=vulgo.pk)).delete()
        elif not vulgo_nome and VulgosThroughModel.objects.filter(Q(pedido_inclusao_id=pk)):
            VulgosThroughModel.objects.filter(Q(pedido_inclusao_id=pk)).delete()

        if outros_nomes:
            for outro_nome in PedidoInclusaoOutroNome.objects.filter(~Q(nome__in=outros_nomes) & Q(pedido_inclusao_id=pk)):
        ...
        elif not outros_nomes and PedidoInclusaoOutroNome.objects.filter(pedido_inclusao_id=pk):
            PedidoInclusaoOutroNome.objects.filter(Q(pedido_inclusao_id=pk)).delete()

        if motivos_inclusao:
            motivos = [motivo.pk for motivo in PedidoInclusaoMotivos.objects.filter(pedido_inclusao_id=UUID(pk))]
            motivos_list = list()
            for motivo in motivos_inclusao:
                pedido_motivo, created = PedidoInclusaoMotivos.objects.get_or_create(
                    norma_juridica=motivo["norma_juridica"],
            ...
            if itens_removidos:
                for pedido in PedidoInclusaoMotivos.objects.filter(Q(id__in=itens_removidos)):
                        NormasJuridicasMotivosThroughModel.objects.filter(Q(motivo=pedido)).delete()
                        PedidoInclusaoMotivos.objects.filter(Q(pk=pedido.pk, pedido_inclusao_id=pk)).delete()
        elif not motivos_inclusao and PedidoInclusaoMotivos.objects.filter(pedido_inclusao_id=pk):
            for pedido in PedidoInclusaoMotivos.objects.filter(Q(pedido_inclusao_id=pk)):
                NormasJuridicasMotivosThroughModel.objects.filter(Q(motivo=pedido)).delete()
    ...
    def get_pessoas(self, request):
        lista_pessoa = list()
        busca = dict()
        busca['cpf'] = trata_telefone(self.request.query_params.get("cpf")) if self.request.query_params.get("cpf") else None
        ...
        lista_pessoa.extend(self.get_list_pessoas(queryset=Interno.objects.filter(excluido=False), 
                                                    tipo='INTERNO', 
                                                    busca=busca, 
                                                    filter_all=filter_all))
        
        lista_pessoa.extend(self.get_list_pessoas(queryset=Servidor.objects.filter(excluido=False), 
                                                    tipo='SERVIDOR', 
                                                    busca=busca, 
                                                    filter_all=filter_all))

        if lista_pessoa:
            lista = ordena_lista(lista_pessoa, self.request.query_params.get('ordering'))
        ...
        retorno = {"count": len(lista_pessoa),
                    "next": None,
                    "previous": None,
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="total-fases",
        url_name="total-fases",
    )
    def get_total_fases(self, request):
        retorno = {
            "pendente": PedidoInclusao.objects.filter(fase_pedido__fase_inicial=True, excluido=False).count(),
            "em_analise": PedidoInclusao.objects.filter(fase_pedido__fase='CGIN', excluido=False).count(),
            "remetido": PedidoInclusao.objects.filter(fase_pedido__fase='REMETIDO', excluido=False).count(),
            "arquivado": PedidoInclusao.objects.filter(fase_pedido__fase='ARQUIVADO', excluido=False).count()
        }
        return Response(retorno, status=status.HTTP_200_OK)
    ...
    def get_fase_pedido_ano(self, request):
        ...
        retorno = {
                ...
                "datasets": [
                    {
                    "label": "Concluídos",
                    "data": self.get_total_pedido_por_fase_mes('ULTIMA_FASE', now.month)
                    },
                    {
                    "label": "Arquivados",
                    ...
                    "data": self.get_total_pedido_por_fases_finais_mes('ULTIMA_FASE', 'DEFERIDO', now.month)
                    },
                    {
                    "label": "Indeferido pelo Juízo Corregedor",
                    "data": self.get_total_pedido_por_fases_finais_mes('ULTIMA_FASE', 'INDEFERIDO', now.month)
                    },
                    {
    ...
    def get_mes_ano(self, data_atual):
        ...
        if data_atual.month == 12:
            ...
            list_atual = [f"{mes} - {data_atual.year}"  for mes in meses[:data_atual.month]]
            list_passado = [f"{mes} - {data_atual.year-1}"  for mes in meses[data_atual.month:]]
    ...
    def get_total_pedido_por_fase_mes(self, fase, mes):
        ...
        pedidos_mes = PedidoInclusao.objects.filter(fase_pedido__fase=fase, 
                                                    data_movimentacao__range=[data_inicio, data_fim],
                                                    excluido=False).annotate(month=TruncMonth('data_movimentacao')).values('data_movimentacao__month').annotate(total=Count('id'))
    ...
    def get_total_parecer_cgin_mes(self, parecer, mes):
        ...
        data_fim = datetime.now()
        data_inicio = data_fim - relativedelta(years=1)
        analises_mes = AnalisePedido.objects.filter(posicionamento=parecer,
                                                    created_at__range=[data_inicio, data_fim],
                                                    excluido=False).annotate(month=TruncMonth('created_at')).values('created_at__month').annotate(total=Count('id'))
    ...
    def get_unidades_parecer_cgin(self, request):
        labels_list = list()
        data_list = list()
        for unidade in PedidoInclusao.objects.filter(excluido=False).values('unidade__nome').order_by('unidade').annotate(count=Count('unidade')):
    ...
    def get_grupos(self, request):
        retorno = dict()
        for pedido in PedidoInclusao.objects.filter(excluido=False).values('estado_solicitante__sigla').order_by('estado_solicitante').annotate(count=Count('estado_solicitante')):
    ...
    def get_list_pessoas(self, queryset, tipo, busca, filter_all):
        ...
        for query in queryset:
            ...
            if encontrado is False:
                ...
                if ((busca['cpf'] and busca['cpf'] in pessoa_cpf) or 
                    (busca['nome'] and busca['nome'] in pessoa_nome) or 
                    (busca['nome_mae'] and busca['nome_mae'] in pessoa_nome_mae)):
                    encontrado = True
            ...
            if encontrado is True:
                ...
                dict_pessoa = {
                    ...
                    "data_nascimento": query.data_nascimento if query.data_nascimento else "",
                    "naturalidade": paises,
...
class PedidoInclusaoOutroNomeViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            return super(PedidoInclusaoOutroNomeViewSet, self).create(request, *args, **kwargs)
        except KeyError:
            return Response(
                {"non_field_errors": mensagens.MSG_ERRO},
                status=status.HTTP_400_BAD_REQUEST,
...
class AnalisePedidoViewSet(LoggingMixin, viewsets.ModelViewSet, Base):
    ...
    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    search_fields = ("parecer", "penitenciaria", "posicionamento", "pedido_inclusao", "ativo")
    filter_fields = ("parecer", "penitenciaria", "posicionamento", "pedido_inclusao", "ativo")
    ordering_fields = ("created_at", "parecer", "penitenciaria", "posicionamento", "pedido_inclusao", "ativo")
    ordering = ("created_at", "parecer", "penitenciaria", "posicionamento", "pedido_inclusao", "ativo")

    def create(self, request, *args, **kwargs):

        with transaction.atomic():
            try:
                pedido = PedidoInclusao.objects.select_for_update().get(pk=self.request.data.get('pedido_inclusao'))
            except Exception:
        ...
        try:
            with transaction.atomic():
                if self.request.data.get('penitenciaria'):
                    ...
                    pedido.save()
                return super(AnalisePedidoViewSet, self).update(request, *args, **kwargs)
        except KeyError:
            return Response(
                {"non_field_errors": mensagens.MSG_ERRO},
                status=status.HTTP_400_BAD_REQUEST,
    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if (fase.fase == "ULTIMA_FASE"):
                ...
                try:
                    movimentacao_pedido = PedidoInclusaoMovimentacao.objects.filter(pedido_inclusao_id=self.request.data.get('pedido_inclusao'))
                    analise = AnalisePedido.objects.get(pedido_inclusao_id=self.request.data.get('pedido_inclusao'))
                except Exception:
                    return Response(
                ...
                if analise and autos_recebidos and autos_remetidos:
                    ...
                    return Response({"non_field_errors": mensagens.ERRO_FINALIZAR},status=status.HTTP_400_BAD_REQUEST,) 

            pedido.save()
            return super(PedidoInclusaoMovimentacaoViewSet, self).create(request, *args, **kwargs)


    def filter_queryset(self, queryset):
        queryset = super(PedidoInclusaoMovimentacaoViewSet, self).filter_queryset(queryset)
