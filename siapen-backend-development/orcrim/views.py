...
from orcrim.serializers import FaccaoCargoSerializer, FaccaoPessoaSerializer, FaccaoSerializer, FaccaoGrupoSerializer
...
class FaccaoPessoaViewSet(LoggingMixin, viewsets.ModelViewSet):
    ...
    filter_fields = ('faccao__nome', 'pessoa__nome',
                     'data_filiacao_faccao', 'data_desfiliacao_faccao')
    ordering_fields = ('faccao__nome', 'pessoa__nome',
                       'data_filiacao_faccao', 'data_desfiliacao_faccao', 'ativo')
...
class FaccaoViewSet(LoggingMixin, viewsets.ModelViewSet, Base):
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            if not Base().check_registro_exists(Faccao, requisicao.get('id')):
                return Response({"detail": mensagens.NAO_ENCONTRADO}, status=status.HTTP_404_NOT_FOUND)
            if self.check_faccao(requisicao):
                return Response({"detail": mensagens.MSG4}, status=status.HTTP_409_CONFLICT)
            ...
            if requisicao.get('ativo') is False and not self.inativa_filhos(requisicao.get('id')):
                return Response({"non_field_errors": "Falha ao inativar registros."}, 
    ...
    def destroy(self, request, pk, *args, **kwargs):
        try:
            ...
            if self.check_faccao_cargos(pk):
                return Response({"non_field_errors": mensagens.MSG16}, status=status.HTTP_400_BAD_REQUEST)
            Faccao.objects.filter(id=pk).update(excluido=True,
    ...
    def filter_queryset(self, queryset):
        ...
        for query in Faccao.objects.filter(Q(excluido=False)):
            ...
            facao_list.extend([trata_campo(estado.nome) for estado in query.estado.all()])
    ...
    def inativa_filhos(self, id):
        try:
            FaccaoGrupo.objects.filter(faccao_id=id, 
                                        excluido=False, 
                                        ativo=True).update(ativo=False)
            FaccaoCargo.objects.filter(faccao_id=id, 
                                        excluido=False, 
                                        ativo=True).update(ativo=False)
            return True
        except Exception:
    ...
    def check_faccao(self, requisicao):
        ...
        return Faccao.objects.filter(Q(nome__iexact=nome,
                                       sigla__iexact=sigla, 
                                       excluido=False) &
                                     ~Q(id=faccao_id)).exists()
    
    def check_faccao_grupo(self, id):
        return FaccaoGrupo.objects.filter(Q(faccao_id=id, 
                                            excluido=False)).exists()
    
    def check_faccao_cargos(self, id):
        return FaccaoCargo.objects.filter(Q(faccao_id=id, 
                                            excluido=False)).exists()
...
class FaccaoGrupoViewSet(LoggingMixin, viewsets.ModelViewSet, Base):
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            if Base().check_registro_excluido(FaccaoGrupo, requisicao.get('id')):
                return Response({"non_field_errors": mensagens.NAO_PERMITIDO},
                                 status=status.HTTP_400_BAD_REQUEST)
            if self.check_faccao_grupo(requisicao):
                return Response({"detail": mensagens.MSG4}, status=status.HTTP_409_CONFLICT)
            if requisicao.get('ativo') and not self.check_faccao(requisicao):
                return Response({"detail": "Não é possível realizar esta operação. A Facção está inativa."},
                                 status=status.HTTP_400_BAD_REQUEST)
    ...
    def destroy(self, request, pk, *args, **kwargs):
        try:
            ...
            if Base().check_registro_excluido(FaccaoGrupo, pk):
                return Response({"non_field_errors": mensagens.NAO_PERMITIDO},
                                 status=status.HTTP_400_BAD_REQUEST)
            FaccaoGrupo.objects.filter(id=pk).update(excluido=True,
                                                     usuario_exclusao=user.get_user(self),
                                                     delete_at=datetime.now())
            return Response({"detail": mensagens.MSG5}, status=status.HTTP_200_OK)
        except Exception:
    ...
    def check_faccao_grupo(self, requisicao):
        nome = check_duplicidade(requisicao.get('nome'))
        return FaccaoGrupo.objects.filter(Q(nome__iexact=nome, 
                                            faccao_id=requisicao.get('faccao'), 
                                            excluido=False) & 
                                            ~Q(id=requisicao.get('id'))).exists()
    
    def check_faccao(self, requisicao):
        faccao = FaccaoGrupo.objects.filter(Q(id=requisicao.get('id'))).first()
        return Faccao.objects.filter(Q(id=faccao.faccao_id,
                                        ativo=True,
                                        excluido=False)).exists()
...
class FaccaoCargoViewSet(LoggingMixin, viewsets.ModelViewSet, Base):
    ...
    def destroy(self, request, pk, *args, **kwargs):
        try:
            ...
            if Base().check_registro_excluido(FaccaoCargo, pk):
                return Response({"non_field_errors": mensagens.NAO_PERMITIDO},
                                 status=status.HTTP_400_BAD_REQUEST)
            FaccaoCargo.objects.filter(id=pk).update(excluido=True,
                                                     usuario_exclusao=user.get_user(self),
                                                     delete_at=datetime.now())
            return Response({"detail": mensagens.MSG5}, status=status.HTTP_200_OK)
        except Exception:
    ...
    def check_faccao_cargo(self, requisicao):
        nome = check_duplicidade(requisicao.get('nome'))
        return FaccaoCargo.objects.filter(Q(nome__iexact=nome, 
                                            faccao_id=requisicao.get('faccao'), 
                                            excluido=False) & 
                                            ~Q(id=requisicao.get('id'))).exists()
